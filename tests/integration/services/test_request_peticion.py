...
class TestRequestPeticion:
    ...
    @pytest.fixture
    def service_add_car_request(
        self, repository_peticion, repository, service_register_car
    ):
        return AddCarRequest(
            repositorio_peticion=repository_peticion,
            repositorio_coche=repository,
            register_car=service_register_car,
        )

    def test_request_peticion(
        self,
        service_register_car,
        repository_peticion,
        repository,
        service_add_car_request,
        app,
    ):
        with app.app_context():
            ...
            assert any(
                [
                    coche
                    for coche in repository.get_all()
                    if coche.modelo.nombre == modelo1.nombre
                    and coche.modelo.marca == modelo1.marca
                    and coche.estado == Estado.DISPONIBLE
                ]
            )
